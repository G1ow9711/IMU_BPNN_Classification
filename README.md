...
第一次阅读本项目，建议从[新手向完整算法文档](docs/算法文档.md)开始。文档按真实数据流解释每一步的直观含义、公式、输入输出、边界处理、复杂度、最终精度和 ESP32-S3 资源预算。当前软件边界和后续真板清单见[软件完成性验收报告](docs/软件完成性验收报告.md)。
...
- 训练引擎在 `PREPARING` 和 `RUNNING` 共用同一动作段证据；运行期累计分类与已锁定动作不一致时，冻结相位与计数，仍继续会话时间和热量积分。START、成功 RESUME、采样 RESET/GAP 都会清除旧证据，禁止跨动作段拼接。
...
- 尚未完成的是开发板实测：固件烧录、AMOLED/触摸、QMI8658、AXP2101、振动马达、BLE 配对与弱信号、实际电流、睡眠唤醒及长时间稳定性。文档和本机构建通过不等同于真板验收通过。
