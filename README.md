...
- 8 个文件分组三折晋级的手腕特征：PCA 主轴换向率、自相关第二峰/第一峰比、回摆形状相关、冲击半高宽、事件前后能量比、恢复时间比、周期峰间隔 CV 和二次谐波比。公式与 Python/C 误差见 `docs/手腕换向与周期特征说明.md`。
...
Round24 起验证六分支 BP。当前 M0 六组输入维度为 `(112,48,24,48,32,38)`，分支输出维度为 `(24,12,8,12,8,16)`，拼接后执行 `80 -> 64 -> 32 -> 11`。Round28 只把三折晋级手腕特征从 2 项扩为 8 项，不改变 M0 分支输出宽度、融合层或训练损失，用于隔离新增特征贡献。实验性 M1 把弱类分支输出增至 24，并使用 `88 -> 64 -> 48 -> 32 -> 24 -> 11` 深窄融合；其 296 维验证结果低于 M0，因此默认关闭。
