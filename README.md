...
本项目使用手腕六轴 IMU 数据识别 11 类健身动作。当前最终算法采用 297 项特征、两个轻量六分支 BP、固定 logits 融合和动作段因果累计；ESP32-S3 资源评估已经通过，两个六分支模型的自动 C 权重导出仍待完成。
...
第一次阅读本项目，建议从[新手向完整算法文档](docs/算法文档.md)开始。文档按真实数据流解释每一步的直观含义、公式、输入输出、边界处理、复杂度、最终精度和 ESP32-S3 资源预算。
