...
嵌入式界面不是“屏幕驱动初始化后画几个控件”。供电、总线、面板异步刷新、触摸坐标、IMU 时间轴和业务状态任何一层不稳定，最终都可能表现为黑屏、故障码、触摸无响应或训练时重启。下面按依赖顺序说明为什么要这样集成。
...
低功耗会主动制造“任务暂停、屏幕关闭、BLE 断开、时钟源变化和唤醒重建”。如果基础 UI 或传感器链尚未稳定，这些现象与真实缺陷外观相同。先在常亮模式证明显示、触摸、采样、BLE 和训练长稳，再逐级关闭资源，才能把每次功耗下降与新增风险一一对应。最终效果是获得可测量、可回退的节电收益，而不是一个偶尔无法唤醒的“低功耗”开关。
