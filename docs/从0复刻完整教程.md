...
```text
Project/
├─ Dataset/                  随仓库发布的 11 类训练 IMU 数据与完整性清单
...
公开仓库已经包含这个目录。克隆完成后可以直接训练；本地新增且不准备公开的数据仍放在被 Git 忽略的 `IMU_Dataset/`。
...
```python
record = ImuRecord(
    Path("Dataset/squat/IMU-2023-05-11-17_21_40_squat.txt"),
    "squat",
    6,
)
```
...
```powershell
Get-Content .\Dataset\<动作>\<文件>.txt -TotalCount 1
```
...
这张图直接引用公开 [`Dataset/`](../Dataset/) 的固定文件级验证划分。每个验证文件先选联合形状 medoid 窗口并对齐客观运动峰，同类文件再等权聚合；实线是文件中位数，阴影是文件间四分位区间。它不是挑一条最好看的标准动作：
...
$$
M=\mathrm{std}(A)+\frac{\mathrm{std}(G)}{200}
$$
...
$$
\Delta E_{\mu\mathrm{kcal}}=
\frac{\mathrm{milliMET}\times W_g\times \Delta t_{\mathrm{ms}}\times 7}
{24\,000\,000}
$$
...
在已经进入项目根的 PowerShell 中，对应相对路径是 `.\Dataset`。文件级哈希、行数、类别统计和数据限制见 [`Dataset/manifest.json`](../Dataset/manifest.json) 与 [`Dataset/README.md`](../Dataset/README.md)。
...
`Dataset/` 是随仓库公开的训练/验证数据根，包含 11 类、189 个动作文件、原始格式说明和 SHA-256 清单。`IMU_Dataset/` 只保存本地新增或尚未取得公开授权的数据，继续由 `.gitignore` 排除。
