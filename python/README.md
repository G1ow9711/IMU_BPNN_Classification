...
| 5. 先生成特征图 | 曲线和分布能在训练前暴露单位错误、饱和、弱类重叠和异常文件 | 开发者理解模型将看到什么，不把神经网络当黑盒 | 图表清单中的数据指纹和文件等权规则 |
...
该顺序带来的核心效果是“失败尽早、定位具体”。例如图表已经显示单位异常时，不应继续训练；Python/C 特征不一致时，不应通过重新训练掩盖部署错误；测试集变差时，也不能回头反复挑参数直到测试结果变好。
