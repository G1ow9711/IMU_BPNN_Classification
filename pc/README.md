...
- `.NET 8 + WPF + MVVM` 六页桌面应用：设备、实时训练、训练总结、历史记录、设置和诊断。
...
WPF 页面、Windows BLE、协议状态机和本地存储具有不同失败模式。七个工程按责任拆分，每层都能单独替换，并能在没有真表时使用替身验证。
...
教程读者可以分别替换蓝牙 transport、会话状态机、仓储或页面，其它层继续沿用原合同。
