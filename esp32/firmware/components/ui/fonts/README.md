...
设备屏幕所有用户可见汉字使用 Noto Sans SC 子集，不再依赖只含拉丁字形的 Montserrat。四个字号分别承担：
...
每个字体使用 `2 bpp` 非压缩位图（`bitmap_format=0`），同时包含 ASCII `0x20～0x7E`、`ui_presenter.c` 当前全部用户可见汉字和常用全角标点。非压缩格式对齐厂家稳定中文字体基线，避免真板对压缩字形解码后出现重影。字符集、文件长度和 SHA-256 记录在 `ui_font_manifest.json`。
...
脚本只从 `esp32/firmware/components/ui/ui_presenter.c` 的 C 字符串字面量提取汉字。修改设备可见文案后必须重新生成；禁止手改三个字体 C 文件中的位图、字形描述或 Unicode 映射。
